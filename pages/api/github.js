export default async (req, res) => {
  if (req.method === 'POST') {
    await fetch('https://api.github.com/rate_limit', { method: 'GET' }).then(
      async (response) => {
        if (!response.ok) {
          throw new Error(
            'Não foi possível estabelecer conexão com os servidores, tente novamente!'
          );
        }
        ...
        if (responseJSON.rate.remaining === 0) {
          ...
          const { followingNumbers, followersNumbers } = await fetch(
          ...
          const followingList = await fetch(
          ...
          const followersList = await fetch(
