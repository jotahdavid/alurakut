...
export default function Home(props) {
  ...
  useEffect(() => {
    ...
    // GET Communities
    fetch('/api/communities', { method: 'GET' }).then(async (response) => {
      ...
      <MainGrid>
            ...
            <form
              onSubmit={(event) => {
                ...
                const newCommunity = {
                  ...
                  imageUrl:
                    communityData.get('imageURL') ||
                    `https://picsum.photos/300?10${communities.length}`,
                };
                ...
                fetch('/api/communities', {
                  ...
                  const responseJSON = await response.json();
                  const { id, creatorSlug, title, imageUrl } =
                    responseJSON.newRecord;
        ...
        <section className="profile-relations-area">
          <ProfileRelations
            title="Seguidores"
            list={followers}
            length={numbersOfFollowers}
          />
          <ProfileRelations
            title="Seguindo"
            list={following}
            length={numbersOfFollowing}
          />
          <ProfileRelations
            title="Comunidades"
            list={communities}
